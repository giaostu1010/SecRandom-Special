...
public partial class ImportPrizePage : UserControl
{
    ...
    private readonly ILogger<ImportPrizePage>? _logger;
    ...
    private void LoadFromTxt(string filePath)
    {
        ...
        for (var i = 0; i < lines.Length; i++)
        {
            ...
            {
                _rawData.Add(new Dictionary<string, string> { { "名称", name } });
            }
        }
    }
    ...
    private void LoadFromJson(string filePath)
    {
        ...
        {
            var row = new Dictionary<string, string> { { "名称", name } };
            
            if (value is System.Text.Json.JsonElement element)
            {
                if (element.TryGetProperty("id", out var idProp))
                    row["编号"] = idProp.ToString();
                if (element.TryGetProperty("count", out var countProp))
                    row["数量"] = countProp.ToString();
                if (element.TryGetProperty("weight", out var weightProp))
                    row["权重"] = weightProp.ToString();
    ...
    {
        // 编号列关键词
        var idKeywords = new[] { "编号", "id", "序号" };
        AutoMapColumn("IdColumnComboBox", idKeywords);

        // 名称列关键词
        var nameKeywords = new[] { "名称", "name", "奖品名称", "奖品" };
        AutoMapColumn("NameColumnComboBox", nameKeywords);

        // 数量列关键词
        var countKeywords = new[] { "数量", "count", "份数" };
        AutoMapColumn("CountColumnComboBox", countKeywords);

        // 权重列关键词
        var weightKeywords = new[] { "权重", "weight", "概率" };
        AutoMapColumn("WeightColumnComboBox", weightKeywords);
    ...
    private void UpdatePreview()
    {
        ...
        var countColumn = GetSelectedColumn("CountColumnComboBox");
        var weightColumn = GetSelectedColumn("WeightColumnComboBox");
        ...
        if (importButton != null) importButton.IsEnabled = !string.IsNullOrEmpty(_currentPoolName);

        // 生成预览数据
        var previewItems = new ObservableCollection<PrizePreviewItem>();
        var maxRows = Math.Min(5, _rawData.Count);

        for (var i = 0; i < maxRows; i++)
        {
            var row = _rawData[i];
            var item = new PrizePreviewItem
            {
                Id = idColumn != null && row.TryGetValue(idColumn, out var id) ? id : (i + 1).ToString(),
                Name = row.TryGetValue(nameColumn, out var name) ? name : "",
                Count = countColumn != null && row.TryGetValue(countColumn, out var count) ? count : "1",
                Weight = weightColumn != null && row.TryGetValue(weightColumn, out var weight) ? weight : "1.0",
    ...
    private async void ImportButton_OnClick(object? sender, RoutedEventArgs e)
    {
        try
        {
            ...
            {
                await ShowMessageAsync("错误", "请先选择要导入的奖池");
                return;
            }
            ...
            if (string.IsNullOrEmpty(nameColumn))
            {
                await ShowMessageAsync("错误", "请选择名称列");
                return;
            }
            ...
            foreach (var row in _rawData)
            {
                ...
                var prize = new PrizeItem
                {
                    Id = int.TryParse(idColumn != null && row.TryGetValue(idColumn, out var idVal) ? idVal : "", out var parsedId) 
                        ? parsedId : id,
                    Name = name,
                    Count = int.TryParse(countColumn != null && row.TryGetValue(countColumn, out var countVal) ? countVal : "1", out var parsedCount) 
                        ? parsedCount : 1,
                    Weight = double.TryParse(weightColumn != null && row.TryGetValue(weightColumn, out var weightVal) ? weightVal : "1.0", out var parsedWeight) 
                        ? parsedWeight : 1.0,
            ...
            if (prizes.Count == 0)
            {
                await ShowMessageAsync("提示", "没有找到有效的奖品数据");
                return;
            }

            // 导入数据
            var count = _service.ImportPrizes(_currentPoolName, _selectedFilePath!, true);
            
            await ShowMessageAsync("成功", $"成功导入 {count} 个奖品到奖池 {_currentPoolName}");
        }
        catch (Exception ex)
        {
            _logger?.LogError(ex, "导入奖品失败");
